...
# 划分训练集和验证集
train_texts, val_texts, train_labels, val_labels = train_test_split(modified_texts, diff_labels, test_size=0.2, random_state=42)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for val_text, val_label in zip(val_texts, val_labels):
            val_label = torch.tensor([val_label]).to(device).squeeze()

            # 使用分词器进行编码
            inputs = tokenizer(val_text, return_tensors="pt").to(device)
            if 'token_type_ids' in inputs:
                del inputs['token_type_ids']
            ...
            # 预测验证集结果
            y_val_predict = npcr(mlp_input, example_input)
            y_val_predict = y_val_predict.squeeze().cpu().numpy()

            # 记录预测和标签
            all_preds.append(y_val_predict)
            all_labels.append(val_label.cpu().numpy())

    # 将预测结果四舍五入为整数量化分数（根据你的问题设计需求）
    rounded_preds = np.round(all_preds).astype(int)
    all_labels = np.round(all_labels).astype(int)

    # 计算 QWK 分数
    qwk_score = cohen_kappa_score(all_labels, rounded_preds, weights="quadratic")
